...
function loadWidget(config) {
	...
	async function zoomModelSize(add = false) {
		if (!modelList) await loadModelList();
		var sizeZoomIndex = Number(sessionStorage.getItem("sizeZoomIndex"));
		if (sizeZoomIndex >= modelList.sizeZoomList.length || !(typeof (sizeZoomIndex) == 'number' && !isNaN(sizeZoomIndex)))
			sizeZoomIndex = 0;
		//缓存上一个index大小，用于可进行最大缩放大小的处理：若上一次未超出配置大小，则当前次超出配置大小，则设置为一次屏幕最大的size
		...
		var size;
		if (outBoundSize >= document.body.clientHeight || outBoundSize >= document.body.clientWidth) {
			//若上一次已大小已经超出配置，则直接回到最小
			if (lastIndexSize + outOffsetPx >= document.body.clientWidth) {
				sizeZoomIndex = 0;
